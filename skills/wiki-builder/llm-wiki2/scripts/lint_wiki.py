...
def lint(root: str) -> int:
    ...
    if index_path.exists():
        ...
        not_in_index = [
            ...
            and str(p.relative_to(wiki_path).with_suffix("")).replace("\\", "/") not in index_text
        ]
        if not_in_index:
    ...
    if log_path.exists() and log_path.is_dir():
        ...
        for p in sorted(log_path.iterdir()):
            ...
            if not m:
                log_issues.append("   {} — filename doesn't match YYYYMMDD.md".format(p.relative_to(root_path)))
                continue
            y, mo, d = m.groups()
            iso = "{}-{}-{}".format(y, mo, d)
