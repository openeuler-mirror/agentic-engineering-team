...
class PlatformAPIClient {
  ...
  _createHttpClient() {
    ...
    if (this.config.token) {
      ...
      } else if (this.config.platform === PLATFORM_TYPES.GITLAB) {
    ...
    // Self-hosted GitLab commonly uses self-signed certs
    let httpsAgent;
    if (this.config.platform === PLATFORM_TYPES.GITLAB) {
  ...
  _handleApiError(error) {
    ...
    if (!error.response) {
      ...
      if (error.code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE' ||
          ...
          (error.message && (error.message.includes('self signed certificate') || error.message.includes('SSL')))) {
        logger.error('SSL证书验证失败，建议：如果是GitLab自签名证书，请联系管理员或配置NODE_TLS_REJECT_UNAUTHORIZED=0');
        return new SSLCertificateError('SSL证书验证失败', { originalError: error, code: error.code });
      }
