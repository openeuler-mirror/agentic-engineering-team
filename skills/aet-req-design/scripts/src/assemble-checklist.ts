...
export function loadComponentChecklist(templateFolder: string, componentName: string): string | null {
  ...
  try {
    ...
    console.error(`Warning: Failed to read component ${componentName} (${filePath}): ${(error as Error).message}`);
    return null;
  }
}

export function assembleChecklist(templateFolder: string): string {
  ...
  try {
    ...
    throw new Error(`Failed to read ${checklistPath}: ${(error as Error).message}`);
  }
