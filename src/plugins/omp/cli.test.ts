...
// omp's cli.ts is a self-contained copy of the opencode plugin's cli.ts (the
// spawn + JSON-parse + runAetSafe error-degrade logic is identical). These
// tests mirror opencode/cli.test.ts to keep both copies under the same
// invariant: --output json is always appended; spawn failures never reject
// out of a hook context (R9: errors are surfaced, not silenced).
