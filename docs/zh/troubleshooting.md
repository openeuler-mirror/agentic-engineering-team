...
**错误信息：** `interactive-revision requires Node.js >= 20 (current: ...)`

**原因：** `aet-req-user-review` 技能的修订脚本（`scripts/revision.mjs`）要求 Node.js >= 20，当前 Node 版本过低。

**解决方案：** 升级 Node.js 到 20 及以上版本后重试。该脚本无降级回退机制。

#### 需求交互式修订报锁冲突

**错误信息：** 锁冲突 / lock conflict

**原因：** 存在并发修订会话，同一快照会话被其他进程占用。

**解决方案：** 等待当前会话结束（锁在会话退出时自动释放），或结束占用会话后从 A1 阶段重新开始修订。
