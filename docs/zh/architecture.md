...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ Aet-Implement   │ ← 实现智能体：TDD 开发 + 功能验证
└────────┬────────┘
         │
         ▼
        PR 创建（用户手动执行 `/aet-pr`）
...
| **Aet-Design** | 设计智能体，需求分析 (RAS)、需求设计 (RDS)、开发计划 (SDD) |
...
> 工作流控制 Agent 的执行顺序和确认点，围栏控制开发时 AI 能改哪些文件。两者是独立的：围栏由 Aet-Design 在设计阶段自动生成在 SDD 中，你只需要在评审时确认；workflow 由你手动编辑配置文件。
...
AET 强制 AI 在设计阶段输出结构化文档（RAS → RDS → SDD），将设计方法最佳实践作为不可篡改的基准上下文，确保开发质量。
...
```
Issue → RAS（需求分析）→ RDS（需求设计）→ SDD（开发计划）→ 实现代码 → 验证报告
```
